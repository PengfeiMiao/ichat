...
public class WebPageTool {
    ...
    public String getWebPage(@ToolMemoryId String userName,
                             @P("Name of the search engine: google, baidu, bing, 360, sogou, quark") String engine,
                             @P("Something need newest information") String query) {
        String webUrl = CrawlerUtil.SEARCH_ENGINES.get(engine.toLowerCase());
        if (webUrl == null) {
            int seek = LocalDateTime.now().getSecond();
            String[] urls = CrawlerUtil.SEARCH_ENGINES.values().toArray(new String[]{});
